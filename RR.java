...
public class RR {
	...
	static int timer = 0;
	static Queue <JobQueue> readyQueue = new LinkedList <JobQueue>( );	
	static Queue <JobQueue> IOqueue = new LinkedList <JobQueue> ();
	public static void run (Queue<JobQueue> pq, int q, String algorithm) {
...
//			System.out.println(temp.jobID + " " + temp.bursts);
			...
			int quantum = q;
			//finish the current cpu bursts or the quantum			
			while (x!=0 && quantum!=0) {
				x--;
				temp.processingTime++;
				quantum--;
				...
				else if (x!=0 && quantum==0 ){
					if (IOcounter == 10 && !IOqueue.isEmpty())	{
						readyQueue.add(IOqueue.remove());
						IOcounter =0;
					}
					temp.bursts.addFirst(x);
					readyQueue.add(temp);
					//continue;
				}
			}	
			//special case when the burst didnt finish but the quantum finished
			if (x!=0 && quantum==0){
				continue;
			}
